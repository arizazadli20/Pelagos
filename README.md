...
- **Auth flow**: login/register write localStorage + `seasentry-auth` cookie; middleware protects ops routes; logout clears session and returns to the landing page.
...
| --- | --- | --- | --- |
| *(none for mock demo)* | — | Landing, auth, dashboard, ops pages | Auth uses browser `localStorage` + the `seasentry-auth` cookie |
...
| Key | Where | Purpose |
| --- | --- | --- |
| `seasentry-auth` | Cookie + localStorage flag | Middleware / client auth gate |
| `seasentry-user` | localStorage | Stored mock user profile (`name`, `email`) |
